...
abstract class SchemaVisitor {
  ...
  String recreateSchema(Schema schema) => dropAll(schema) + createAll(schema);
...
class SqlVisitor
    implements SchemaVisitor, TableVisitor, ExprVisitor, QueryVisitor {
  ...
  String dropTable(Table table) => 'Generic drop single table ${table.name}';
...
''';
  ...
  String _fromJoinClauses(Query query) {
    ...
    List clauses = [];
    final tablesNotInJoins =
        query.tables.where((t) => !tablesInJoins.contains(t));
    clauses.add(tablesNotInJoins.map((t) => t.name).join(',\n  '));
    clauses.add(query.joins.map((j) => j.toString()).join('\n  '));
    return clauses.join('\n  ');
  }

  _returns(Query query) => query.returns.map((e) => e.aliased);
